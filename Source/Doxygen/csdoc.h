...
/*
 * A class to translate doxygen comments into CSDoc style comments.
 */
class CSDocConverter : public DoxygenTranslator {
public:
  CSDocConverter(int flags = 0);
  String *makeDocumentation(Node *node);

protected:
  /*
   * Used to properly format CSDoc-style command
   */
   ...
   */
  typedef void (CSDocConverter::*tagHandler) (DoxygenEntity &tag, std::string &translatedComment, std::string &arg);
  ...
  /*
   * Print extended csdoc command, like {@code ...} or {@literal ...}
   * arg - command name
   */
